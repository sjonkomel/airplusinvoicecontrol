#!/usr/bin/env python3
"""
Corrected LARS Validator for Your Actual File Format
====================================================

This validator uses the CORRECTED field definitions from lars_config_corrected.py
based on your actual 30000 record structure.

Usage:
    from validator_corrected import CorrectedLARSValidator
...
from lars_config_corrected import CORRECTED_FIELD_DEFINITIONS, CORRECTED_VALIDATION_RULES, get_record_type_from_line, get_field_definitions
...
class CorrectedLARSValidator:
    """
    Corrected validator for your actual LARS file format.
    Uses the corrected field definitions based on your 30000 record analysis.
    """
    
    def __init__(self):
        self.file_path = None
        self.lars_file = None
        self.field_definitions = CORRECTED_FIELD_DEFINITIONS
        self.validation_rules = CORRECTED_VALIDATION_RULES
    ...
    def _validate_record(self, record: LARSRecord, field_defs: Dict) -> List[ValidationError]:
        ...
        for field_name, field_def in field_defs.items():
            ...
            if field_type == 'date':
                ...
                # For your format, amounts might have implied decimals
                # Don't validate format strictly, just check it's numeric
                if not self._validate_decimal(field_value):
                    # Don't report as error for now, since your amounts might be integers
                    pass
    ...
    def _validate_cross_records(self, lars_file: LARSFile) -> List[ValidationError]:
        ...
        # Check for transaction records (30000)
        transaction_records = [r for r in lars_file.records if r.record_type == '30000']
        if not transaction_records:
            errors.append(ValidationError(
                ...
                severity=ErrorSeverity.WARNING,
                message="No transaction records (type 30000) found",
                can_auto_fix=False
            ))
        
        # Check for continuity of transaction records
        # Each 30000 should be followed by a 30001
        for i in range(len(lars_file.records) - 1):
            current = lars_file.records[i]
            next_rec = lars_file.records[i + 1]
            
            if current.record_type == '30000' and next_rec.record_type != '30001':
                errors.append(ValidationError(
                    line_number=current.line_number,
                    field_name="RECORD_SEQUENCE",
                    error_type="missing_continuation",
                    severity=ErrorSeverity.INFO,
                    message=f"Transaction record (30000) at line {current.line_number} not followed by continuation (30001)",
    ...
    def _validate_decimal(self, value: str) -> bool:
        """Validate a decimal number string - more lenient for your format"""
        try:
            value = value.strip()
            if not value:
                return False
    ...
    def print_30000_analysis(self, lars_file: LARSFile = None):
        """Print detailed analysis of 30000 records"""
        if lars_file is None:
            if self.lars_file is None:
        ...
        print("\n" + "=" * 80)
        print("30000 RECORD ANALYSIS")
        print("=" * 80)
        
        transaction_records = [r for r in lars_file.records if r.record_type == '30000']
        
        if not transaction_records:
            print("No 30000 records found")
            return
        
        print(f"Found {len(transaction_records)} 30000 records\n")
...
def validate_corrected_lrs_file(file_path: str) -> Dict[str, Any]:
    """
    Quick function to validate a LRS file with corrected configuration.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(file_path):
            ...
            # Print 30000 analysis
            validator.print_30000_analysis(lars_file)
        else:
            print(f"File not found: {file_path}")
    else:
        print("Usage: python validator_corrected.py <path_to_lrs_file>")
