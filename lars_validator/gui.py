#!/usr/bin/env python3
"""
LARS File Validator and Repair Tool - GUI
==========================================

Graphical user interface for the LARS file validator using Tkinter.
"""
...
from main import (
    LARSValidator, LARSFile, LARSVersion, ValidationError, ErrorSeverity, LARSRecord
)
...
class LARSValidatorGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.current_file: Optional[LARSFile] = None
        self.current_file_path: Optional[str] = None
        self.fixed_file: Optional[LARSFile] = None
    ...
    def setup_file_tab(self):
        ...
        for i, (label_text, field_name) in enumerate(info_fields):
            ...
            self.file_info_labels[field_name] = ttk.Label(
                info_frame, textvariable=value_var
            )
            self.file_info_labels[field_name].grid(
                row=row, column=col*2+1, sticky=tk.W, padx=5, pady=2
            )
            setattr(self, f"{field_name}_var", value_var)
    ...
    def setup_repair_tab(self):
        ...
        # Manual repair frame
        manual_frame = ttk.LabelFrame(repair_frame, text="Manual Repair", padding="10")
        manual_frame.pack(fill=tk.BOTH, expand=True, pady=5)
        
        # Record selection
        record_frame = ttk.Frame(manual_frame)
        record_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(record_frame, text="Select Record:").pack(side=tk.LEFT)
        ...
        self.record_combo.pack(side=tk.LEFT, padx=5)
        self.record_combo.bind("<<ComboboxSelected>>", self.load_record_details)
        
        # Record details
        self.record_details_text = scrolledtext.ScrolledText(
            manual_frame,
            height=12,
            wrap=tk.WORD,
            state=tk.DISABLED
        )
        self.record_details_text.pack(fill=tk.BOTH, expand=True)
    ...
    def setup_export_tab(self):
        ...
        # Export as
        ttk.Label(options_frame, text="Export as:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        output_frame.grid(row=3, column=0, sticky=tk.W, pady=5)
...
FILE FORMAT:
LARS files are fixed-width text files with the following structure:
- Header record (type 01) - File information
- Transaction records (type 02, 03, etc.) - Individual transactions
- Footer record (type 99) - File summary
...
COMMON ERRORS:
- Missing or invalid record types
...
AUTOMATIC FIXES:
The application can automatically fix many common issues:
- Date format corrections
- Decimal number formatting
- Integer formatting
- Record count updates

For more information about LARS format, visit:
https://www.airplus.com
        ...
        # Version info
        version_frame = ttk.Frame(help_frame)
        version_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(version_frame, text="Version: 1.0.0").pack(side=tk.LEFT)
        ttk.Label(version_frame, text="| ").pack(side=tk.LEFT, padx=5)
        
        # Link to more info
        info_link = ttk.Label(
            version_frame,
            text="Visit AirPlus website",
            foreground="blue",
            cursor="hand2"
        )
        info_link.pack(side=tk.LEFT)
        info_link.bind("<Button-1>", lambda e: webbrowser.open("https://www.airplus.com"))
...
"""
    ...
    def filter_errors(self, event=None):
        ...
        for item in self.errors_tree.get_children():
            ...
            self.errors_tree.item(item, tags=('show',) if show else ('hide',))
    ...
    def update_record_combo(self):
        """Update the record selection combo"""
        if not self.current_file:
            return
        
        # Get invalid records
        invalid_records = [
            r for r in self.current_file.records 
            if not r.is_valid and r.record_type not in ['01', '99']
        ]
        
        record_options = []
        for record in invalid_records:
            record_options.append(f"Line {record.line_number} - Type {record.record_type}")
        
        if record_options:
            self.record_combo['values'] = record_options
            self.record_combo.current(0)
            self.record_combo.config(state=tk.NORMAL)
            self.load_record_details()
        else:
            self.record_combo['values'] = []
            self.record_combo.config(state=tk.DISABLED)
    
    def load_record_details(self, event=None):
        """Load details for the selected record"""
        if not self.current_file:
            return
        
        selection = self.record_var.get()
        if not selection:
            return
        
        # Extract line number from selection
        match = re.match(r"Line (\d+) - Type (\d+)", selection)
        if not match:
            return
        ...
        # Find the record
        record = None
        for r in self.current_file.records:
        ...
        details = f"Record Details - Line {record.line_number}\n"
        details += "=" * 50 + "\n\n"
        details += f"Record Type: {record.record_type}\n"
        details += f"Status: {'VALID' if record.is_valid else 'INVALID'}\n\n"
        ...
        details += "\nOriginal Line:\n"
        details += f"  {record.original_line}\n"
        
        if record.errors:
            details += "\nErrors:\n"
            for error in record.errors:
                details += f"  - {error.error_type}: {error.message}\n"
                if error.can_auto_fix:
                    details += f"    Suggested fix: {error.suggested_fix}\n"
        
        self.record_details_text.config(state=tk.NORMAL)
        self.record_details_text.delete(1.0, tk.END)
        self.record_details_text.insert(tk.END, details)
        self.record_details_text.config(state=tk.DISABLED)
...
"""
    ...
    def clear_file(self):
        ...
        # Clear info labels
        for field in ['version', 'total_records', 'valid_records', 'invalid_records', 
                     'total_errors', 'critical_errors', 'errors', 'warnings', 'auto_fixable']:
            var_name = f"{field}_var"
            if hasattr(self, var_name):
                var = getattr(self, var_name)
                var.set("-")
