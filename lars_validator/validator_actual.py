#!/usr/bin/env python3
"""
Actual LARS Validator for Your File Format
==========================================

This validator is specifically designed for your actual LARS file format
with record types: 20000, 20001, 20002, 30000, 30001, 40000, 59000, 70000.

Usage:
    from validator_actual import ActualLARSValidator
...
from lars_config_actual import ACTUAL_FIELD_DEFINITIONS, ACTUAL_VALIDATION_RULES, get_record_type_from_line, get_field_definitions
...
class ActualLARSValidator:
    """
    Validator for your actual LARS file format with record types:
    20000 (Header), 20001 (Header line 1), 20002 (Header line 2),
    30000 (Transaction), 30001 (Transaction line 1),
    40000 (Flight info), 59000 (Ticket detail), 70000 (Footer)
    """
    
    def __init__(self):
        self.file_path = None
        self.lars_file = None
        self.field_definitions = ACTUAL_FIELD_DEFINITIONS
        self.validation_rules = ACTUAL_VALIDATION_RULES
    ...
    def _validate_record(self, record: LARSRecord, field_defs: Dict) -> List[ValidationError]:
        ...
        for field_name, field_def in field_defs.items():
            ...
            if field_name == 'RECORD_TYPE' and field_value:
                valid_types = ['20000', '20001', '20002', '30000', '30001', '40000', '59000', '70000']
                if field_value not in valid_types:
                    errors.append(ValidationError(
                        ...
                        severity=ErrorSeverity.ERROR,
                        message=f"Invalid record type: {field_value}. Valid types: {', '.join(valid_types)}",
                        expected_value=f"One of: {', '.join(valid_types)}",
                        actual_value=field_value,
                        can_auto_fix=False
                    ))
            
            elif field_name == 'CURRENCY_CODE' and field_value:
                if not re.match(r'^[A-Z]{3}$', field_value):
                    errors.append(ValidationError(
                        line_number=record.line_number,
                        field_name=field_name,
                ...
                if not re.match(r'^\d{13,19}$', field_value):
                    errors.append(ValidationError(
                        line_number=record.line_number,
                        field_name=field_name,
                        error_type="invalid_card_number",
                        severity=ErrorSeverity.WARNING,
                        message=f"Invalid card number format: {field_value}",
...
def validate_actual_lrs_file(file_path: str) -> Dict[str, Any]:
    """
    Quick function to validate an actual LRS file and return summary.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(file_path):
            summary = validate_actual_lrs_file(file_path)
        ...
        print("Usage: python validator_actual.py <path_to_lrs_file>")
